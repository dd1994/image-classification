...
class INatBaseDataModule(pl.LightningDataModule):
    def __init__(self, data_dir: str = './data/tiny', valid_dir= './data/tiny', id_map_file_path = '', valid_id_map_file_path = '',batch_size: int = 32, num_workers: int = 3, input_size: int = 448, num_classes = 51, rrc_scale_min: float = 0.3, random_erase_prob: float = 0.25, re_scale_min: float = 0.02, re_scale_max: float = 0.2, rebalance_last_epochs: int = 0, rebalance_floor_count: int = 100, trivial_augment_prob: float = 1.0, random_rotation_degrees: float = 30.0, random_rotation_prob: float = 0.1):
        ...
        self.trivial_augment_prob = trivial_augment_prob
        ...
        self.transform = {
            'train': v2.Compose([
                ...
                v2.RandomApply([v2.RandomRotation(degrees=self.random_rotation_degrees)], p=self.random_rotation_prob),
                v2.RandomApply([TrivialAugmentWide()], p=self.trivial_augment_prob),
    ...
    def train_dataloader(self):
        ...
        return DataLoader(self.train_dataset, batch_size=self.batch_size,
                          shuffle=True, num_workers=self.num_workers, persistent_workers=False, prefetch_factor=2)

    def val_dataloader(self):
        return DataLoader(self.val_dataset, batch_size=self.batch_size*3,
                          shuffle=False, num_workers=self.num_workers*2, persistent_workers=False, prefetch_factor=2)
