...
2. **`RandomResizedCrop` 的 `scale` 参数**：当前默认 `scale=(0.08, 1.0)` 最小裁剪到原图 8%，对细粒度分类偏小。建议 `scale=(0.2, 1.0)`，保留更多原始分辨率的局部细节（羽毛纹理、翅脉、花瓣边缘等判别特征）。

3. **`RandomVerticalFlip`**：部分物种（如蝙蝠、蜘蛛）上下不对称，水平翻转不适用。加在 `TrivialAugmentWide` 之后：
   ```python
   v2.RandomVerticalFlip(p=0.5)
   ```

4. **`RandomErasing` 参数调小**：当前可能偏激进，建议更保守的参数防过度擦除关键判别区域：
   ```python
